...
// The actual store
DS.StubberAdapter = DS.Adapter.extend({
  namespace: null,
  identityField: 'id',
  // Helper functions
  getIdentifyField: function(){
    return this.identityField||'id';
  },
  singularize: function(term){
    return inflection.singularize(term);
  },
  pluralize: function(term){
    return inflection.pluralize(term);
  },
  getSingularName: function(type){
    var uri = this.buildURL(type);
    var name = uri.split('/').pop();
    return this.singularize(name);
  },
  getPluralName: function(type){
    var uri = this.buildURL();
    var name = uri.split('/').pop();
    return this.pluralize(name);
  },
  getURI: function(type){
    var uri = this.namespace;
    if(!uri){
      uri = '/api/'+this.pluralize(type.toString().split('.').pop().toLowerCase());
    }
    return uri;
  },
  buildSingularURL: function(type, identity){
    var urlParts = this.getURI(type).split('/');
    urlParts.push(this.singularize(urlParts.pop()));
    if(!Ember.isNone(identity)){
      urlParts.push(identity);
    }
    return urlParts.join('/');
  },
  buildPluralURL: function(type){
    var urlParts = this.getURI(type).split('/');
    urlParts.push(this.pluralize(urlParts.pop()));
    return urlParts.join('/');
  }, 
  buildURL: function(type, identity, record){
    if(identity==='bulk'){
      return this.buildPluralURL(type);
    }else if(!Ember.isNone(identity)){
      return this.buildSingularURL(type, identity);
    }else{
      return this.getURI(type);
    }
  },
  // Directly copied from standard REST_adapter, with some changes
  rootForType: function(type) {
    var serializer = get(this, 'serializer');
    return serializer.rootForType(type);
  },
  shouldSave: function(record) {
    var reference = Ember.get(record, '_reference');
  ...
  dirtyRecordsForHasManyChange: function(dirtySet, record, relationship) {
    var embeddedType = Ember.get(this, 'serializer').embeddedType(record.constructor, relationship.secondRecordName);
  ...
  _dirtyTree: function(dirtySet, record) {
    ...
    Ember.get(this, 'serializer').eachEmbeddedRecord(record, function(embeddedRecord, embeddedType) {
  ...
  createRecord: function(store, type, record) {
    ...
    return this.ajax(this.buildSingularURL(type), "POST", {
      data: data
    }).then(function(json){
  ...
  createRecords: function(store, type, records) {
    ...
    return this.ajax(this.buildPluralURL(type), "POST", {
      data: data
    }).then(function(json) {
  ...
  updateRecord: function(store, type, record) {
    ...
    return this.ajax(this.buildSingularURL(type, id, record), "PUT",{
      data: data
    }).then(function(json){
  ...
  updateRecords: function(store, type, records) {
    ...
    return this.ajax(this.buildPluralURL(type, "bulk"), "PUT", {
      data: data
    }).then(function(json) {
  ...
  deleteRecord: function(store, type, record) {
    ...
    return this.ajax(this.buildSingularURL(type, id, record), "DELETE").then(function(json){
  ...
  deleteRecords: function(store, type, records) {
    ...
    data[plural] = [];
    records.forEach(function(record) {
      data[plural].push(serializer.serializeId( Ember.get(record, 'id') ));
    });

    return this.ajax(this.buildPluralURL(type, 'bulk'), "DELETE", {
      data: data
    }).then(function(json){
  ...
  find: function (store, type, id) {
    var uri = this.buildURL(type, id);
    var identityField = this.getIdentifyField();
    $.ajax({
      url:      uri,
      dataType: 'json',
      context:  store,
      success:  function(response){
        var item = (response||{})[(response||{}).root||'']||{};
        this.load(type, item[identityField], item);
      }
    });
  },
  findAll: function(store, type) {
    var uri = this.buildPluralURL(type);
    var identityField = this.getIdentifyField();
    $.ajax({
      url:      uri,
      dataType: 'json',
      context:  store,
      success:  function(response){
        var items = (response||{})[(response||{}).root||'']||[];
        var ids = [], l=items.length, i;
        for(i=0; i<l; i++){
          ids.push(items[i][identityField]);
        }
        this.loadMany(type, ids, items);
      }
    });
  }
});
